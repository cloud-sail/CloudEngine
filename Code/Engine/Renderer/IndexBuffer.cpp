...
{
	return GetSize() / GetStride(); // ? cannot shrink the size
}
...
IndexBuffer::IndexBuffer(DX12Renderer* renderer, ID3D12Device* device, unsigned int size)
	: m_renderer(renderer)
	, m_device(device)
	, m_size(size)
{
...
void IndexBuffer::Upload(ID3D12GraphicsCommandList* cmdList, void const* data, unsigned int dataSize)
{
	if (dataSize > m_size)
	{
		Resize(dataSize);
	...
	m_currentResourceState = static_cast<unsigned int>(D3D12_RESOURCE_STATE_INDEX_BUFFER);

	//-----------------------------------------------------------------------------------------------
	// deplete the upload buffer after uploading
	//if (m_uploadBuffer)
	//{
	//	m_renderer->EnqueueDeferredRelease(m_uploadBuffer);
	//	m_uploadBuffer = nullptr;
...
void IndexBuffer::CreateDefaultBuffer()
{
	if (m_defaultBuffer)
	{
		m_renderer->EnqueueDeferredRelease(m_defaultBuffer);
	...
	if (FAILED(hr))
	{
		ERROR_AND_DIE("Could not create index buffer on default heap");
	}

#ifdef _DEBUG
	m_defaultBuffer->SetName(L"Default heap of index buffer");
#endif // _DEBUG

	m_currentResourceState = static_cast<unsigned int>(D3D12_RESOURCE_STATE_COMMON);
}

//void IndexBuffer::CreateUploadBuffer(unsigned int size)
//{
//	if (m_uploadBuffer)
//	{
//		m_renderer->EnqueueDeferredRelease(m_uploadBuffer);
...
//	if (FAILED(hr))
//	{
//		ERROR_AND_DIE("Failed to create index buffer on upload heap");
//	}
//
//#ifdef _DEBUG
//	m_uploadBuffer->SetName(L"Upload heap of index buffer");
//#endif // _DEBUG
//
