...
void Renderer::DrawIndexedVertexArray(std::vector<Vertex_PCU> const& verts, std::vector<unsigned int> const& indexes)
{
	m_immediateVBO->m_stride = sizeof(Vertex_PCU);
	CopyCPUToGPU(verts.data(), static_cast<unsigned int>(verts.size()) * m_immediateVBO->GetStride(), m_immediateVBO);
...
void Renderer::DrawIndexedVertexArray(std::vector<Vertex_PCUTBN> const& verts, std::vector<unsigned int> const& indexes)
{
	m_immediateVBO->m_stride = sizeof(Vertex_PCUTBN);
	CopyCPUToGPU(verts.data(), static_cast<unsigned int>(verts.size()) * m_immediateVBO->GetStride(), m_immediateVBO);
	CopyCPUToGPU(indexes.data(), static_cast<unsigned int>(indexes.size()) * m_immediateIBO->GetStride(), m_immediateIBO);
...
void Renderer::AttachGeometryShader(Shader* shader, char const* shaderName)
{
...
ID3D11VertexShader* Renderer::CreateVertexShader(std::vector<unsigned char> const& byteCode)
{
...
ID3D11PixelShader* Renderer::CreatePixelShader(std::vector<unsigned char> const& byteCode)
{
...
ID3D11GeometryShader* Renderer::CreateGeometryShader(std::vector<unsigned char> const& byteCode)
{
...
ID3D11InputLayout* Renderer::CreateInputLayout(std::vector<unsigned char> const& byteCode, VertexType type /*= VertexType::VERTEX_PCU*/)
{
...
void Renderer::BindPrimitiveTopology()
{
...
void Renderer::SetPerFrameConstants(PerFrameConstants const& perframeConstants)
{
...
void Renderer::SetModelConstants(Mat44 const& modelToWorldTransform /*= Mat44()*/, Rgba8 const& modelColor /*= Rgba8::OPAQUE_WHITE*/)
{
...
void Renderer::SetLightConstants(Vec3 const& sunDirection /*= Vec3(2.f, -1.f, -1.f)*/, float sunIntensity /*= 0.85f*/, float ambientIntensity /*= 0.35f*/)
{
...
void Renderer::SetSamplerMode(SamplerMode samplerMode, int slot /*= 0*/)
{
...
void Renderer::SetEngineConstants(int debugInt /*= 0*/, float debugFloat /*= 0.f*/)
{
...
// Renderer Startup
//-----------------------------------------------------------------------------------------------
void Renderer::InitializeDebugModule()
{
	//-----------------------------------------------------------------------------------------------
	// Create debug module 
...
void Renderer::CreateDefaultShaderAndTexture()
{
	//-----------------------------------------------------------------------------------------------
